 ...
 void sendDataToIbusSimulation(char *pDataSend, int DataLenS)
{
 	...
 	if(m_class_big == (jint)5 || (m_class_big == (jint)15)){//如果消息大类是5，就是app消息，那么就把这个app消息的源地址和目标地址进行交换
		...
		if(m_class_small == (jint)0x1000){//如果消息小类是0x1000，则为触发配置消息
			jint triggerNumber = (jint)(*(p_data_temp+6)) | ((jint)(*(p_data_temp+7))<<8);//去掉前四个字节的仲裁场以及两个备用字节
		...
		if(m_class_small == (jint)0x1003){//如果消息小类是0x1003，则为设置消息滤波器消息
			setModuleRegisterBuf(m_destination_addr, (p_data_temp+6),4);//将收到的属性值设置到对应IP地址的模块中
		}
		...
		if(m_class_small == (jint)0x1003){//如果消息小类是0x1003，则为获取消息滤波器消息
			...
			getModuleRegisterBuf(m_destination_addr, &p_register_buf, &bufLength);//获取相应IP值的module当中的属性值，这里使用二级指针来进行指针赋值
			
			char *p_data_temp = (char *)malloc(10);//分配十个字节用来存放消息内容，这个是属性获取消息，其回复是确认的10字节
			...
			(*(MID_BITS *)p_data_temp).m_dst_addr = temp.m_dst_addr;//完成源地址和目的地址的交换
 			...
 			putDataToModule(m_destination_addr, p_data_temp, 10);//将需要发送的数据放入虚拟前端模块当中，待发送
 		}
 	}
}
